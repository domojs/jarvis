                ...
                $.getJSON('http://localhost/api/weather/forecast/'+(cmd.time && cmd.time.isValid() ? cmd.time.daysFromNow()+1 : 0), function(forecast){
                    switch(cmd.target)
                    {
                        case 'temperature':
                            ...
                            callback(null, 'il va faire entre '+forecast.low.celsius+'° et '+forecast.high.celsius+'°');
                            else
                            callback(null, 'il fais entre '+forecast.low.celsius+'° et '+forecast.high.celsius+'°');
                            break;
                        case 'rain':
                            if(cmd.action=='is')
                            {
